...
$project = "ConstructionLine.CodingChallenge"
$packageId = "Test"
...
try {
	...
	if (-not $SkipBackend) {
		...
		# Run tests		
		Invoke-TestCore -ProjectNameFilter "*Tests*" -Config $config -CoverletExcludes "[*.*Tests?]*", "[LazyCache*]*" -Skip:$SkipTests
		...
		New-ZipPackage -id "$packageId.$project" -version $BuildNumber -FolderToZip $publishedFolder -outputFolder $ReleaseDir -RemoveFolder
	}
}
finally {
	ReportResults
